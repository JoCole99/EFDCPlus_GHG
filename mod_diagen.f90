...
! ----------------------------------------------------------------------
  ...
  Call Broadcast_Array(SMJAQCH4,   master_id)  !SWWCGHG
  Call Broadcast_Array(SMJAQN2O,   master_id)  !SWWN2O
  Call Broadcast_Array(SMJAQN2ON,   master_id)  !SWWN2O
  Call Broadcast_Array(SMJAQN2OD,   master_id)  !SWWN2O
  Call Broadcast_Array(SMJGCH4,    master_id)  !SWWCGHG
  Call Broadcast_Array(SMCH4SAT,   master_id)  !SWWCGHG
  Call Broadcast_Array(SMCH4REF,   master_id)  !SWWCGHG
  Call Broadcast_Array(SMJCH4CO2,  master_id)  !SWWCGHG
  Call Broadcast_Array(SMDGFCCO2,  master_id)  !SWWCGHG
  Call Broadcast_Array(SMJDEN,  master_id)     !SWWN2O
  Call Broadcast_Array(SMJNIT,  master_id)     !SWWN2O
...
1111  CONTINUE
  ...
  REAL    :: SMBST1, RSMSS, CSODMSM, SMKHPOC !SWWCGHG 
  ...
  !$OMP DO PRIVATE(ND, LF, LL, L, M)        
   DO ND=1,NDM  
    ...
    SMKHPOC = 1.0 !SWWCGHG: POC half-sat constant for poc mineralization
    DO M=1,NSMG
      DO L=LF,LL
        ! ***         Organic M    Algal & POM Sources
        SMPOC(L,M) = (SMPOC(L,M) + SMDFC(L,M)*SMDTOH(ISMZMAP(L))) / (SMW2DTOH(ISMZMAP(L)) + SMTDCD(ISMT(L),M)*DTWQ*1.1 + 1.E-18) !SWWCGHG: Modify the consumption of POC from POC mineralization
        !SMPOC(L,M) = (SMPOC(L,M) + SMDFC(L,M)*SMDTOH(ISMZMAP(L))) / (SMW2DTOH(ISMZMAP(L)) + SMTDCD(ISMT(L),M)*DTWQ*1.1/MIN(2.0 , SMKHPOC+SMPOC(L,M)+1.E-18) + 1.E-18) !SWWCGHG: Modify the POC consumption from POC mineralization
    ...
    DO L=LF,LL
      ...
      !SMDGFC(L) = SMHSED(ISMZMAP(L)) * (SMTDCD(ISMT(L),1)*SMPOC(L,1)/(SMKHPOC + SMPOC(L,1) + 1.E-18)  + SMTDCD(ISMT(L),2)*SMPOC(L,2)/(SMKHPOC + SMPOC(L,2) + 1.E-18)) !SWWCGHG: Michaelis-Menten equation of POC
      SMDGFCCO2(L) = (10.*0.001+SMHSED(ISMZMAP(L)))*(SMTDCD(ISMT(L),1)*SMPOC(L,1)  + SMTDCD(ISMT(L),2)*SMPOC(L,2)) !SWWCGHG CO2 poduced from POC mineralization
      !SMDGFCCO2(L) = (10.*0.001+SMHSED(ISMZMAP(L)))*(SMTDCD(ISMT(L),1)*SMPOC(L,1)/(SMKHPOC + SMPOC(L,1) + 1.E-18)  + SMTDCD(ISMT(L),2)*SMPOC(L,2)/(SMKHPOC + SMPOC(L,2) + 1.E-18)) !SWWCGHG CO2 poduced from POC mineralization
      !SMTDCD(IT,M) = SMKPOC(M) * SMTHKC(M)**TT20
      ...
      XSMO20(L) = MIN(MAX( WQV(L,KSZ(L),IDOX), 0.1 ), 3.0)  !SWWN2O
      SMTMP(L) = SMBST(L)
  ...
  DO ND=1,NDM  
    ...
    DO L=LF,LL
      IF( SCB(L) > 0.5 )THEN
        ...
        ! *** Ammonia
        SK1NH4SM = ( SMKNH4(IZ)*SMTDNH4(ISMT(L)) * XSMO20(L) ) / ( (SMKMO2N+XSMO20(L)+ 1.E-12) * (SMKMNH4+SM1NH4(L)) ) !nitrification
        ...
        ! *** Nitrate
        SK1NO3SM = SMK1NO3(IZ)*SMTDNO3(ISMT(L)) !denitrification
        A1NO3SM  = SMKL12(L) + SMW2(IZ)
        A2NO3SM  = SMKL12(L)
        RK2NO3SM = SMK2NO3(IZ)*SMTDNO3(ISMT(L)) !denitrification
		...
		ELSE
          ! *** Methane for salinity <= SMCSHSCH
          !SMCH4S  = 10 * (10.0 + HP(L) + SMHSED(IZ)) * SMTD1CH4(ISMT(L))
		  SMCH4S  = SMO2C * 1.013*10000 *(10. + HP(L) + SMHSED(IZ))*(0.05708-0.001545*SMT(L)+0.00002069*SMT(L)**2) * 12 / (8.3145*(SMT(L)+273.15)+1.E-18) !SWWCGHG: Modification of CH4 ebullition conc threshold
          SMK1CH4 = SMTD2CH4(ISMT(L))
        ENDIF

        ! *** BACK SUBSTITUTION TO GET SURFACE MASS TRANSFER COEFFICIENT (SMSS) 得到所有变量的浓度
        ...
        SMNSOD(L) = -RNSODSM
        !SMJNIT(L) = RJNITSM
        SMJDEN(L) = RJDENSM  !RJDENSM = SK1NO3SM/(RSMSS+1.E-18)*SM1NO3(L) + RK2NO3SM*SM2NO3(L) !反硝化速率（NO3消耗）
        SMJAQN2ON(L) = 0.75*44./14.*RJNITSM*SMKHO2N/(SMKHO2N+XSMO20(L)+1.E-12) * XSMO20(L)/(SMKHO2D+XSMO20(L)+1.E-12)           !硝化产N2O  SWWN2O
        SMJAQN2OD(L) = 0.5*44./14.*SK1NO3SM/(RSMSS+1.E-18)*SM1NO3(L) * XSMO20(L)/(SMKHO2D+XSMO20(L)+1.E-12)                     !反硝化产N2O  SWWN2O
        SMJAQN2O(L)  = SMJAQN2ON(L) + SMJAQN2OD(L)
        SMJAQN2(L) = 0.75*28./14.*RJNITSM*SMKHO2N/(SMKHO2N+XSMO20(L)+1.E-12) * SMKHO2D/(SMKHO2D+XSMO20(L)+1.E-12)  &            !硝化产N2  SWWN2O
                     + 0.5*28./14.*(SK1NO3SM/(RSMSS+1.E-18)*SM1NO3(L)*SMKHO2D/(SMKHO2D+XSMO20(L)+1.E-12) + RK2NO3SM*SM2NO3(L))  !反硝化产N2  SWWN2O

        ! *** Determine COD based on H2S or CH4
        SMJAQH2S(L) = AQJH2SSM + AQJCH4SM
		SMJAQCH4(L) = AQJCH4SM   !SWWCGHG: Save the diffusion flux to SMJAQCH4(L)
        SMJCH4CO2(L) = CSODSM !SWWCGHG: CO2 produced from CH4 oxidation
        SMJGCH4(L) = GJCH4SM !SWWCGHG: Save the ebullition flux to SMJGCH4(L)
        SMCH4SAT(L) = SMCH4S !SWWCGHG: Save the CH4 ebullition conc threshold to SMCH4SAT(L)
        SMCH4REF(L) = 0.5*(SMO2JC - SMO2NO3*RJDENSM)/SMKL12(L) !SWWCGHG: Monitoring variable of CH4 conc in sediment
  ...
  RJNITSM = RRNH4 * RSM1NH4 !硝化速率（NH4消耗）
  
  ! *** NO3
  RRNO3 = SK1NO3SM/(RSMSS+ 1.E-18)
  A11NO3 = RSMSS + A1NO3SM + RRNO3
  !B11NO3 = RJNITSM + RSMSS*B1NO3SM
  B11NO3 = RJNITSM*XSMO20(L)/(SMKHO2N+XSMO20(L)+1.E-12) + RSMSS*B1NO3SM  !SWWN2O:上层NO3生成
  B22NO3 = B2NO3SM

  CALL SOLVSMBE(RSM1NO3,RSM2NO3,A11NO3,A22NO3SM,A1NO3SM,A2NO3SM,B11NO3,B22NO3)

  RJDENSM = RRNO3*RSM1NO3 + RK2NO3SM*RSM2NO3 !反硝化速率（NO3消耗）
  ...
  IF( SAL(L,KSZ(L)) > SMCSHSCH )THEN
    ...
    GJCH4SM = 0.0
  ELSE
    CSODMSM = MIN( SQRT(2*SMKL12(L)*SMCH4S*SMJ2H2S), SMJ2H2S )

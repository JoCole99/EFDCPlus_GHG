...
MODULE WATERQUALITY!
    ...
    DO K=1,KC
      ...
      DO NAL = 1, NALGAE
        IF( ALGAES(NAL).ISMOBILE )THEN
          WQCHL(L,K) = WQCHL(L,K) + WQV(L,K,22+NAL)*ALGAES(NAL).WQCHLA
        ENDIF
      ENDDO
      IF( IWQSRP == 1 )THEN
  ...
  DO NAL = 1, NALGAE
    IF( .NOT. ALGAES(NAL).ISMOBILE )THEN
      ...
      ELSE
        ...
        HEIGHT_MAC(L,NAL) = ALGAES(NAL).HEIGHTMAX - ALGAES(NAL).HEIGHTMIN
      ENDIF
    ENDIF
  ENDDO
  ...
  WQCONSTIT(IFCB) = 'FCB'
  WQCONSTIT(ICO2) = 'CO2'
  WQCONSTIT(INO2) = 'NO2'  !SWWN2O
  WQCONSTIT(IN2O) = 'N2O'  !SWWN2O
  WQCONSTIT(IN2)  = 'N2'   !SWWN2O
  ...
  IF( IWQZPL > 0 )THEN
    DO NW = 1,NZOOPL
      WRITE(SNUM,'(I1)') NW
      WQCONSTIT(22+NALGAE+NW) = 'ZOO'//SNUM
    ENDDO
  ENDIF
  ...
  if( process_id == master_id )then
    ...
    ISKINETICS(19)=1 !SWWN2O
    ISKINETICS(20)=1 !SWWN2O
    ISKINETICS(21)=1 !SWWN2O
    ISKINETICS(22)=1 !SWWN2O
    If( ISWQLVL == 0  )then
    ...
    CALL SKIPCOM(0119,'*')  !SWWN2O: Read parameters
    READ(0119,*) WQKHORDOC,WQKCH4,WQKHCH4,SMKHO2N,SMKHO2D,WQTHDEN,WQTDEN
    WRITE(*,*) WQKHORDOC,WQKCH4,WQKHCH4,SMKHO2N,SMKHO2D,WQTHDEN,WQTDEN
    CALL SKIPCOM(0119,'*')
    READ(0119,*) WQNH4NO2M,WQNO2NO3M,WQNO3NO2M,WQNO2N2ONM,WQNO2N2ODM,WQN2ON2M
    WRITE(*,*) WQNH4NO2M,WQNO2NO3M,WQNO3NO2M,WQNO2N2ONM,WQNO2N2ODM,WQN2ON2M
    CALL SKIPCOM(0119,'*')
    READ(0119,*) WQKHNH4_NH4NO2,WQKHDO_NH4NO2
    CALL SKIPCOM(0119,'*')
    READ(0119,*) WQKHNH4_NO2N2ON,WQKHNO2_NO2N2ON
    CALL SKIPCOM(0119,'*')
    READ(0119,*) WQKHNO2_NO2NO3,WQKHDO_NO2NO3
    WRITE(*,*) WQKHNH4_NH4NO2,WQKHDO_NH4NO2, WQKHNH4_NO2N2ON,WQKHNO2_NO2N2ON, WQKHNO2_NO2NO3,WQKHDO_NO2NO3
    CALL SKIPCOM(0119,'*')
    READ(0119,*) WQKHNO3_NO3NO2,WQKHDOC_NO3NO2,WQKHDO_NO3NO2
    CALL SKIPCOM(0119,'*')
    READ(0119,*) WQKHNO2_NO2N2OD,WQKHDOC_NO2N2OD,WQKHDO_NO2N2OD
    CALL SKIPCOM(0119,'*')
    READ(0119,*) WQKHN2O_N2ON2,WQKHDOC_N2ON2,WQKHDO_N2ON2
    WRITE(*,*) WQKHNO3_NO3NO2,WQKHDOC_NO3NO2,WQKHDO_NO3NO2, WQKHNO2_NO2N2OD,WQKHDOC_NO2N2OD,WQKHDO_NO2N2OD, WQKHN2O_N2ON2,WQKHDOC_N2ON2,WQKHDO_N2ON2
    ...
    !---------------------------------------------------------------------------
    DO NW = 1,NWQV
      IF (NW>=20 .AND. NW<=22) THEN !SWWN2O 这里根据模型实际的时间序列数来调整
        NWQCSR(NW)=7
        CYCLE
      END IF
    ...
    DO N1 = 1, ICOUNT   !  fson_value_count(pointsource)
      ...
      Do NW = 1, NWQV
        IF (NW>=20 .AND. NW<=22) THEN !SWWN2O
          XPSL(NW)=0.0
          CYCLE
        ENDIF
  ...
  Call Broadcast_Scalar(WQNH4NO2M,        master_id) !SWWN2O
  Call Broadcast_Scalar(WQNO2NO3M,        master_id)
  Call Broadcast_Scalar(WQNO3NO2M,        master_id)
  Call Broadcast_Scalar(WQNO2N2ONM,       master_id)
  Call Broadcast_Scalar(WQNO2N2ODM,       master_id)
  Call Broadcast_Scalar(WQN2ON2M,         master_id)
  Call Broadcast_Scalar(WQKHNH4_NH4NO2,   master_id)  !SWWN2O
  Call Broadcast_Scalar(WQKHDO_NH4NO2,    master_id)
  Call Broadcast_Scalar(WQKHNH4_NO2N2ON,  master_id)  !SWWN2O
  Call Broadcast_Scalar(WQKHNO2_NO2N2ON,  master_id)
  Call Broadcast_Scalar(WQKHNO2_NO2NO3,   master_id)  !SWWN2O
  Call Broadcast_Scalar(WQKHDO_NO2NO3,    master_id)
  Call Broadcast_Scalar(WQKHNO3_NO3NO2,   master_id)  !SWWN2O
  Call Broadcast_Scalar(WQKHDOC_NO3NO2,   master_id)
  Call Broadcast_Scalar(WQKHDO_NO3NO2,    master_id)
  Call Broadcast_Scalar(WQKHNO2_NO2N2OD,  master_id)  !SWWN2O
  Call Broadcast_Scalar(WQKHDOC_NO2N2OD,  master_id)
  Call Broadcast_Scalar(WQKHDO_NO2N2OD,   master_id)
  Call Broadcast_Scalar(WQKHN2O_N2ON2,    master_id)  !SWWN2O
  Call Broadcast_Scalar(WQKHDOC_N2ON2,    master_id)
  Call Broadcast_Scalar(WQKHDO_N2ON2,     master_id)
  Call Broadcast_Scalar(SMKHO2N,          master_id)  !SWWN2O
  Call Broadcast_Scalar(SMKHO2D,          master_id)
  Call Broadcast_Scalar(WQTDEN,           master_id)
  Call Broadcast_Scalar(WQTHDEN,           master_id)
  ...
  Call Broadcast_Scalar(WQKHORDOC,   master_id)  !SWWCGHG
  Call Broadcast_Scalar(WQKHDNN,     master_id)
  Call Broadcast_Scalar(WQAANOX,     master_id)  !SWWCGHG
  Call Broadcast_Scalar(WQKCH4,      master_id)  !SWWCGHG
  Call Broadcast_Scalar(WQKHCH4,     master_id)  !SWWCGHG
  Call Broadcast_Array(TDENIT,     master_id)  !SWWCGHG
  Call Broadcast_Array(THR,        master_id)  !SWWCGHG
  Call Broadcast_Array(TCH4OXID,   master_id)  !SWWCGHG
  Call Broadcast_Array(TALGAE,     master_id)  !SWWCGHG
  Call Broadcast_Array(TN2ON2,     master_id)  !SWWN2O
  Call Broadcast_Array(TNO2N2ON,   master_id)  !SWWN2O
  Call Broadcast_Array(TNO2N2OD,   master_id)  !SWWN2O
  ...
  ! **************************************************************************
  ! *** Set up look-up table for temperature dependency over -10 癈 to 50 癈
  WQTDMIN =-10
  WQTDMAX = 50
  WTEMP = WQTDMIN
  ...
  DO M = 1,NWQTD
    ...
    ! *** TEMPERATURE ADJUSTED RATES FOR NITRIFICATION  !SWWN2O
    IF( WTEMP > WQTNIT )THEN
      !WQTDNIT(M) = WQNITM*EXP(-WQKN2*(WQTNIT-WTEMP)**2)
      WQTDNIT(M) = EXP(-WQKN2*(WQTNIT-WTEMP)**2)
    ELSE
      !WQTDNIT(M) = WQNITM*EXP(-WQKN1*(WTEMP-WQTNIT)**2)
      WQTDNIT(M) = EXP(-WQKN1*(WTEMP-WQTNIT)**2)
    ENDIF
    ! *** TEMPERATURE ADJUSTED RATES FOR DENITRIFICATION  !SWWN2O
    WQTDDEN(M) = WQTHDEN**(WTEMP-WQTDEN)
  ...
  DO NAL = 1,NALGAE
    IF( ISMOB(NAL) == 0 )THEN  ! *** ALGAES(NAL).ISMOBILE is only available after calling ALGAECONTROL - DKT
      ...
      TVARWQ = WQV(1,1,22+NAL)  
      DO L = 1,LCM
        WQV(L,KSZ(L),22+NAL)  = TVARWQ
        WQVO(L,KSZ(L),22+NAL) = TVARWQ
      ENDDO
    ENDIF
  ENDDO
...
1112  CONTINUE
  ...
  if( process_id == master_id  .and. NALGAE > 0 )then
    ...
    DO NAL = 1, fson_value_count(phytogroups)
      ...
      IF( .NOT. ALGAES(NAL).ISMOBILE )THEN
        ...
        Call fson_get(item, "minimum_height",                               ALGAES(NAL).HEIGHTMIN)      ! *** Mimimum height above the cell bottom to allow macrophyte/periphyton growth (m)
        Call fson_get(item, "maximum_height",                               ALGAES(NAL).HEIGHTMAX)      ! *** Maximum height above the cell bottom to allow macrophyte/periphyton growth (m)
  ...
  Do NAL = 1, NALGAE
     ...
     Call Broadcast_Scalar(ALGAES(NAL).HEIGHTMIN,   master_id)                                               
     Call Broadcast_Scalar(ALGAES(NAL).HEIGHTMAX,   master_id)                                               
  ...
  ELSE
    DO NAL = 1,NALGAE
      IF( ALGAES(NAL).ISMOBILE )THEN
        ! *** Phytoplankton
        WQCHL(1,1) = WQCHL(1,1) + WQV(1,1,22+NAL)*ALGAES(NAL).WQCHLA
      ENDIF
    ENDDO
  ENDIF
  ...
  ! Subroutine: Subroutine WQCSR
  !
  !> @details  READ TIME SERIES FILES FOR WQ CONCENTRATIONS  时间序列
  !
  !---------------------------------------------------------------------------!
  !---------------------------------------------------------------------------!
  SUBROUTINE WQCSR2
  ...
  INTEGER NDAY,TDAY
  REAL :: WQPHTM !SWWCGHG: Array to read the pH time series
  ...
  DO NW = 1,NALGAE
    WRITE(SNUM,'(I2.2)')NW
    FNWQSR(NW+22)='wqalgsr'//SNUM//'.inp'
  ENDDO
  ! *** Zooplankton
  DO NW = 1,NZOOPL
     WRITE(SNUM,'(I2.2)')NW
     FNWQSR(NW+22+NALGAE)='wqzoosr'//SNUM//'.inp'
  ENDDO
...
20 CONTINUE
  ...
  !
      ...
      DO NAL = 1, NALGAE
        IF( .NOT. ALGAES(NAL).ISMOBILE .AND. (K >= LAYERBOT(NAL,L) .AND. K <= LAYERTOP(NAL,L)) )THEN
          ...
          XMRM = XMRM + ALGAES(NAL).WQKDCALM(1) * WQVO(L,K,22+NAL)
        ENDIF
      ENDDO
  ...
  !
        ...
        WQP16(L) = - (WQVREA + WQWREA) * HPKI(L,K)* WQTDKR(IWQT(L),IZ)
        WQKRDOS(L) = - WQP16(L)*WQDOS(L)
      ELSE
        WQP16(L) = 0.0
      ENDIF
  ...
  REAL PPCDO, TMP22, WQA22, WQA22C, WQA22D, WQA22G, WQCDDOC,Ka0, WQH, WQKOH, WQKH  !SWWCGHG
        ...
        IF( .NOT. ALGAES(NAL).ISMOBILE )THEN
          DO K = KSZ(L),KC
            IF( K >= LAYERBOT(NAL,L) .AND. K <= LAYERTOP(NAL,L) )THEN
              ...
              WQVA1C = 1.0 / (1.0 - WQAC)
              WQV(L,K,22+NAL) = (WQV(L,K,22+NAL) + WQAC*WQV(L,K,22+NAL))*WQVA1C
              WQV(L,K,22+NAL) = MAX(WQV(L,K,22+NAL), ALGAES(NAL).WQBMIN(IWQZMAP(L,K)))
              WQO(L,22+NAL) = WQVO(L,K,22+NAL) + WQV(L,K,22+NAL)
            ENDIF
          ENDDO
        ENDIF
      ENDDO
  ...
  DO ND = 1,NDM
    ...
    DO K = KC,1,-1  !层循环
      ...
      DO LP = 1,LLWET(K,ND)
        ...
        DO NAL = 1,NALGAE
          IF( ALGAES(NAL).ISMOBILE  )THEN
            ...
            !SWWCGHG: PH  limiation for algae growth
            WQH=10.**(-WQPH(INT(TIMEDAY)))
            WQKOH=10.**(-13)*(8*TWQ(L)**2-500*TWQ(L)+8000)
            WQKH=10.**(-7)*(-5*TWQ(L)**3+300*TWQ(L)**2-5000*TWQ(L)+30000)
            WQF5PH(NAL) = WQH/(WQH+WQKOH+WQH**2/(WQKH+1.E-18)+1.E-18)
        ...
        DO NAL = 1,NALGAE
          IF( .NOT. ALGAES(NAL).ISMOBILE .AND. (K >= LAYERBOT(NAL,L) .AND. K <= LAYERTOP(NAL,L)) )THEN
            ...
            XMRM = WQV(L,K,22+NAL)*HPK(L,K)          ! *** Convert WQV (gC/M3) to a density: XMRM (gC/M2)  
        ...
        DO NAL = 1,NALGAE
          IF( ALGAES(NAL).ISMOBILE )THEN
            ...
            WQPA(L,NAL) = ALGAES(NAL).WQPMA (IMWQZT(L)) * WQF1N(NAL)*WQF2I(NAL)*WQTDG(IWQT(L),NAL)*WQF5PH(NAL)      ! *** Nutrient & Temperature Adjusted SWWCGHG: Add WQF5PH(NAL)
      ...
      DO LP = 1,LLWET(K,ND)
        ...
        WQNO3NO2(L) = WQNO3NO2M * RNO3WQ(L)/(WQKHNO3_NO3NO2+RNO3WQ(L)+1.E-18) * WQKHDO_NO3NO2/(WQKHDO_NO3NO2+O2WQ(L)+1.E-18) * WQV(L,K,IDOC)/(WQKHDOC_NO3NO2+WQV(L,K,IDOC)+1.E-18) * WQTDDEN(IWQT(L))   !SWWN2O: NO3→NO2
        WQNO2N2OD(L) = WQNO2N2ODM * WQV(L,K,INO2)/(WQKHNO2_NO2N2OD+WQV(L,K,INO2)+1.E-18) * WQKHDO_NO2N2OD/(WQKHDO_NO2N2OD+O2WQ(L)+1.E-18) * WQV(L,K,IDOC)/(WQKHDOC_NO2N2OD+WQV(L,K,IDOC)+1.E-18) * WQTDDEN(IWQT(L))   !SWWN2O: NO2→N2O
        WQN2ON2(L) = WQN2ON2M * WQV(L,K,IN2O)/(WQKHN2O_N2ON2+WQV(L,K,IN2O)+1.E-18) * WQKHDO_N2ON2/(WQKHDO_N2ON2+O2WQ(L)+1.E-18) * WQV(L,K,IDOC)/(WQKHDOC_N2ON2+WQV(L,K,IDOC)+1.E-18) * WQTDDEN(IWQT(L))   !SWWN2O: N2O→N2
      ENDDO  
      ...
      DO LP = 1,LLWET(K,ND)
        ...
        ENDDO
        WQNH4NO2(L) =  WQNH4NO2M * O2WQ(L)/(WQKHDO_NH4NO2+O2WQ(L) + 1.E-18) * RNH4WQ(L)/(WQKHNH4_NH4NO2+RNH4WQ(L)+1.E-18) * WQTDNIT(IWQT(L))  !SWWN2O: NH4→NO2
        WQNO2N2ON(L) = WQNO2N2ONM * WQV(L,K,INO2)/(WQKHNO2_NO2N2ON+WQV(L,K,INO2)+1.E-18) * RNH4WQ(L)/(WQKHNH4_NO2N2ON + RNH4WQ(L) + 1.E-18) * WQTDNIT(IWQT(L))  !SWWN2O: NO2→N2O
        WQNO2NO3(L) = WQNO2NO3M * WQV(L,K,INO2)/(WQKHNO2_NO2NO3+WQV(L,K,INO2)+1.E-18) * O2WQ(L)/(WQKHDO_NO2NO3+O2WQ(L)+1.E-18) * WQTDNIT(IWQT(L))  !SWWN2O: NO2→NO3
      ENDDO
      ...
      DO LP = 1,LLWET(K,ND)
        ...
        !************* CO2 parameters Cs=Kh*p  10^(-CDOSATIDX(L))-Kh  mol/(L·atm)

        !************* CH4 parameters SWWCGHG:saturation concentration of dissolved CH4 in top layer
        WQMTHS(L) = 1308.*2.*16. * 10.**(-9.0) * 2.7182818**(2675.*(1/(TWQ(L) + 273.15)-1/298.15)) !VB EVALUATING CONC OF CH4 IN G/M^3 
        !************* CH4 parameters
        
        !************* N2O parameters SWWN2O:saturation concentration of dissolved N2O in top layer
        WQN2OS(L) = 24717.*0.67*44. * 10.**(-9.0) * 2.7182818**(1750.*(1/(TWQ(L) + 273.15)-1/298.15)) !VB EVALUATING CONC OF N2O IN G/M^3 
        !************* CH4 parameters
        
        !************* N2 parameters SWWN2O:saturation concentration of dissolved N2 in top layer
        WQN2S(L) = 0.648*0.756*28. * 2.7182818**(1750.*(1/(TWQ(L) + 273.15)-1/298.15)) !VB EVALUATING CONC OF N2 IN G/M^3 
        !************* N2 parameters
        ...
        IF( K == KC )THEN       
          ...
          WQP16(L)    = -(WQVREA + WQWREA) * WQTDKR(IWQT(L),IZ)   ! *** Reaeration rate (1/day)  
          WQKRDOS(L)  = -WQP16(L)*WQDOS(L)                        ! *** O2 Flux due to DO saturation (offset by current DO below)
          WQP19(L)    = WQP16(L)*((32./44.)**0.25)                ! *** Kr FOR CO2 ANALOGOUS TO WQP16 ; 44 = MOL WT OF CO2
          WQKRCDOS(L) = -WQP19(L) * WQCDOS(L)                     ! *** EVALUATING Kr*SAT CONC OF CO2
		  WQP18(L)    = WQP16(L)*((32./16.)**0.25)                ! SWWCGHG: -Kw of CH4
          WQP21(L)    = WQP16(L)*((32./44.)**0.25)                ! SWWN2O: -Kw of N2O
          WQP22(L)    = WQP16(L)*((32./28.)**0.25)                ! SWWN2O: -Kw of N2
          WQKW18(L) = -WQP18(L) ! SWWCGHG
          WQKW19(L) = -WQP19(L) ! SWWCGHG
          WQKW21(L) = -WQP21(L) ! SWWN2O
          WQKW22(L) = -WQP22(L) ! SWWN2O
        ELSE
          WQKRDOS(L)  = 0.0
          WQKRCDOS(L) = 0.0  
          WQP16(L)    = 0.0  
          WQP18(L)    = 0.0              !VB Kr FOR CO2 IS ZERO FOR CELLS NOT AT THE SURFACE
		  WQP19(L)    = 0.0              !SWWCGHG
          WQP21(L)    = 0.0              !SWWN2O
          WQP22(L)    = 0.0              !SWWN2O
        ENDIF  
        ...
        IF( .NOT. ALGAES(NAL).ISMOBILE )THEN
          DO LP = 1,LLWET(K,ND)
            ...
            IF( K >= LAYERBOT(NAL,L) .AND. K <= LAYERTOP(NAL,L) )THEN
              ...
              IF( WQV(L,K,22+NAL) > ALGAES(NAL).WQBMAX )THEN
              ...
              WQVA1C = 1.0 / (1.0 - WQAC)
              WQV(L,K,22+NAL) = (WQV(L,K,22+NAL) + WQAC*WQV(L,K,22+NAL))*WQVA1C*SMAC(L)  
              WQV(L,K,22+NAL) = MAX(WQV(L,K,22+NAL), ALGAES(NAL).WQBMIN(IMWQZ))*SMAC(L)  
              WQO(L,22+NAL) = WQVO(L,K,22+NAL) + WQV(L,K,22+NAL)
      ...
      DO NAL = 1,NALGAE
        IF( ALGAES(NAL).ISMOBILE )THEN
          ...
          DO LP = 1,LLWET(K,ND)
            ...
            ! ***   PT_SRC_LOADS    VOLUME  
            WQRC = WQWPSL(L,K,22+NAL) * VOLWQ(L)                             ! *** Point source load rate multiplied by inverse cell volume  g/m^3/t
            WQRR(L) = WQV(L,K,22+NAL) + DTWQ*WQRC + WQAC*WQV(L,K,22+NAL)     ! *** Transported biomass conc. (CALWQC) + point source load rate X time step + growth rate X previous biomass conc.
          ...
          IF( K /= KC )THEN
            IF( ALGAES(NAL).WQWS(IMWQZT2(2)) > 0.0 )THEN
              ...
              DO LP = 1,LLWET(K,ND)
                L = LKWET(LP,K,ND)  
                WQRR(L) = WQRR(L) + DTWQO2*WQBSET(L,2,NAL)*WQVO(L,K+1,22+NAL)  ! *** Biomass conc. + DtX(1/t)* biomass conc.
              ENDDO
            ELSE
              ! *** From below
              DO LP = 1,LLWET(K,ND)
          ...
          ELSE  ! K == KC
            DO LP = 1,LLWET(K,ND)
              ...
              WQRC = (WQWDSL(L,KC,22+NAL) + WQATML(L,KC,22+NAL))*VOLWQ(L)    ! *** Atmospheric loading mass per time / cell volume
              WQRR(L) = WQRR(L) + DTWQ*WQRC                                  ! *** Biomass conc. + Dt*loading rate per unit volume
            ENDDO
          ENDIF  
          DO LP = 1,LLWET(K,ND)
            L = LKWET(LP,K,ND)  
            WQV(L,K,22+NAL) = WQRR(L)*WQKK(L)
            WQV(L,K,22+NAL) = MAX(WQV(L,K,22+NAL), ALGAES(NAL).WQBMIN(1))    ! *** Apply a user specified minimum concentration
            WQO(L,22+NAL)   = WQVO(L,K,22+NAL) + WQV(L,K,22+NAL)             ! *** Depth totaled biomass conc = old biomass conc in cell + biomass conc from this iteration
          ENDDO
        ENDIF
      ENDDO

      !*******************************************************************************************************************************
      ! *** NOW COMPUTE KINETICS FOR EACH CONSTITUENT  水质变量过程
      ...
      IF( ISKINETICS(1) == 1 )THEN  
        DO LP = 1,LLWET(K,ND)
          ...
          DO NAL = 1,NALGAE
            IF( ALGAES(NAL).ISMOBILE )THEN
              ! *** Phytoplankton
              WQA4 = WQA4 + ALGAES(NAL).WQFCRP*WQPR(L,NAL)*WQO(L,22+NAL)
      ...
      IF( ISKINETICS(2) == 1 )THEN  
        DO LP = 1,LLWET(K,ND)
          ...
          DO NAL = 1,NALGAE
            IF( ALGAES(NAL).ISMOBILE )THEN
              ! *** Phytoplankton
              WQA5 = WQA5 + ALGAES(NAL).WQFCLP*WQPR(L,NAL)*WQO(L,22+NAL)
      ...
      IF( ISKINETICS(3) == 1 )THEN  
        DO LP = 1,LLWET(K,ND)
          ...
          !WQD6 = -(WQKHR(L) + WQNO3NO2(L)) *DTWQO2
          !WQKK(L) = 1.0 / (1.0 - WQD6)
          ...
          DO NAL = 1,NALGAE
            IF( ALGAES(NAL).ISMOBILE  )THEN
              ...
              WQA6 = WQA6 + ( WQA6A(NAL)*WQBM(L,NAL) + ALGAES(NAL).WQFCDP*WQPR(L,NAL) )*WQO(L,22+NAL)
            ENDIF
            IF( .NOT. ALGAES(NAL).ISMOBILE .AND. (K >= LAYERBOT(NAL,L) .AND. K <= LAYERTOP(NAL,L)) )THEN
              ! *** Macrophytes and periphyton
              WQA6A(NAL) = ALGAES(NAL).WQFCDB + (1.0 - ALGAES(NAL).WQFCDB)*(ALGAES(NAL).WQKHRA(IZ)/(ALGAES(NAL).WQKHRA(IZ) + O2WQ(L) + 1.E-18)) 
          ...
          WQRR(L) = WQV(L,K,IDOC) + DTWQ*WQR6 - WQKHR(L)*DTWQO2*WQVO(L,K,IDOC)/(WQKHORDOC+WQVO(L,K,IDOC)+1.E-18) + DTWQO2*(WQA6 + WQKRPC(L)*WQO(L,IROC) + WQKLPC(L)*WQO(L,ILOC))  !SWWCGHG: Michaelis-Menten equation of DOC
          WQRR(L) = WQRR(L) - 0.5*DTWQ*(12./14.*WQNO3NO2(L) + 12./14.*WQNO2N2OD(L) + 12./44.*WQN2ON2(L)) !SWWN2O: source/sink term of DOC
          ! *** Coupling to zooplankton - DKT              
      ...
      IF( ISKINETICS(4) == 1 )THEN  
        DO LP = 1,LLWET(K,ND)
          ...
          DO NAL = 1,NALGAE
            IF( ALGAES(NAL).ISMOBILE  )THEN
              ! *** Phytoplankton
              WQA7A(NAL) = (ALGAES(NAL).WQFPRB*WQBM(L,NAL) + ALGAES(NAL).WQFPRP*WQPR(L,NAL)) * WQO(L,22+NAL)
      ...
      IF( ISKINETICS(5) == 1 )THEN  
        DO LP = 1,LLWET(K,ND)
          ...
          DO NAL = 1,NALGAE
            IF( ALGAES(NAL).ISMOBILE  )THEN
              ! *** Phytoplankton
              WQA8A(NAL) = (ALGAES(NAL).WQFPLB*WQBM(L,NAL) + ALGAES(NAL).WQFPLP*WQPR(L,NAL)) * WQO(L,22+NAL)
      ...
      IF( ISKINETICS(6) == 1 )THEN  
        DO LP = 1,LLWET(K,ND)
          ...
          DO NAL = 1,NALGAE
            IF( ALGAES(NAL).ISMOBILE )THEN
              ! *** Phytoplankton
              WQA9A(NAL) = (ALGAES(NAL).WQFPDB*WQBM(L,NAL) + ALGAES(NAL).WQFPDP*WQPR(L,NAL)) * WQO(L,22+NAL)
        ...
        DO LP=1,LLWET(K,ND)
          ...
          DO NAL = 1,NALGAE
            IF( ALGAES(NAL).ISMOBILE /= 0  )THEN
              ! *** Phytoplankton
              WQA10A(NAL) = (ALGAES(NAL).WQFPIB*WQBM(L,NAL) + ALGAES(NAL).WQFPIP*WQPR(L,NAL) - WQPA(L,NAL))*WQO(L,22+NAL)
      ...
      IF( ISKINETICS(8) == 1 )THEN  
        DO LP = 1,LLWET(K,ND)
          ...
          DO NAL = 1,NALGAE
            IF( ALGAES(NAL).ISMOBILE  )THEN
              ! *** Phytoplankton
              WQA11A(NAL) = (ALGAES(NAL).WQFNRB*WQBM(L,NAL) + ALGAES(NAL).WQFNRP*WQPR(L,NAL))*ALGAES(NAL).WQANCA*WQO(L,22+NAL)  
      ...
      IF( ISKINETICS(9) == 1 )THEN  
        DO LP = 1,LLWET(K,ND)
          ...
          DO NAL = 1,NALGAE
            IF( ALGAES(NAL).ISMOBILE  )THEN
              ! *** Phytoplankton
              WQA12A(NAL) = (ALGAES(NAL).WQFNLB*WQBM(L,NAL) + ALGAES(NAL).WQFNLP*WQPR(L,NAL))*ALGAES(NAL).WQANCA*WQO(L,22+NAL)  
      ...
      IF( ISKINETICS(10) == 1 )THEN  
        DO LP = 1,LLWET(K,ND)
          ...
          DO NAL = 1,NALGAE
            IF( ALGAES(NAL).ISMOBILE  )THEN
              ! *** Phytoplankton
              WQA13A(NAL) = (ALGAES(NAL).WQFNDB*WQBM(L,NAL) + ALGAES(NAL).WQFNDP*WQPR(L,NAL))*ALGAES(NAL).WQANCA*WQO(L,22+NAL)  
        ...
        DO LP = 1,LLWET(K,ND)
          L = LKWET(LP,K,ND)  
          WQV(L,K,IDON) = WQRR(L)*WQKK(L)
          WQO(L,IDON)   = WQVO(L,K,IDON) + WQV(L,K,IDON)
        ENDDO  
      ENDIF  
      
      ! ****  PARAM 11  NHX - ammonia nitrogen  g N/(m3·d)
        ...
        DO LP = 1,LLWET(K,ND)
          ...
          DO NAL = 1,NALGAE
            IF( ALGAES(NAL).ISMOBILE  )THEN
              ...
              WQA14 = WQA14 + WQA14A(NAL)*ALGAES(NAL).WQANCA*WQO(L,22+NAL)
            ENDIF
            IF( .NOT. ALGAES(NAL).ISMOBILE .AND. (K >= LAYERBOT(NAL,L) .AND. K <= LAYERTOP(NAL,L)) )THEN
              ! *** Macrophytes and periphyton
              WQA14A(NAL) = ALGAES(NAL).WQFNIB*WQBM(L,NAL) + ALGAES(NAL).WQFNIP*WQPR(L,NAL) - WQPN(L,1)*WQPA(L,NAL)  
          ...
          WQRR(L) = WQV(L,K,INHX) + DTWQ*WQRR(L)  - DTWQ*(WQNH4NO2(L)+0.5*WQNO2N2ON(L)) + DTWQO2*(WQA14 + WQKDON(L)*WQO(L,IDON)) !SWWN2O source/sink term of NH4

          ! *** Coupling to zooplankton - DKT
          IF( IWQZPL > 0 ) WQRR(L) = WQRR(L) + DTWQ*SNH4Z(L,K)
          
          WQV(L,K,INHX) = WQRR(L)*WQKK(L)
          WQO(L,INHX)   = WQVO(L,K,INHX) + WQV(L,K,INHX)
        ENDDO  
      ENDIF  

      ! ****  PARAM 12  NOX - nitrate nitrogen  g N/(m3·d)
      IF( ISKINETICS(12) == 1 )THEN 
        ...
        DO LP = 1,LLWET(K,ND)
          ...
          DO NAL = 1,NALGAE
            IF( ALGAES(NAL).ISMOBILE  )THEN
              ! *** Phytoplankton
              WQA15A(NAL) = (WQPN(L,NAL) - 1.0)*WQPA(L,NAL) * ALGAES(NAL).WQANCA * WQO(L,22+NAL)  
          ...
          !WQB15 = WQV(L,K,INOX) + DTWQ*WQRR(L) + DTWQO2*( WQA15 - WQANDC*WQNIT(L)*WQO(L,IDOC) + WQNIT(L)*WQO(L,INHX))
          WQB15 = WQV(L,K,INOX) + DTWQ*WQRR(L) - DTWQ*WQNO3NO2(L) + DTWQ*WQNO2NO3(L) + DTWQO2*WQA15 !SWWN2O source/sink term of NO3
                                                  !反硝化            !硝化
          WQV(L,K,INOX) = WQB15
          ...
          DO LP = 1,LLWET(K,ND)
            ...
            DO NAL = 1,NALGAE
              IF( ALGAES(NAL).ISILICA )THEN
                WQA16D = WQA16D + (ALGAES(NAL).WQFSPD*WQBM(L,NAL) + ALGAES(NAL).WQFSPP*WQPR(L,NAL)) * ALGAES(NAL).WQASC * WQO(L,22+NAL)
      ...
      IF( ISKINETICS(14) == 1 )THEN  
        IF( IWQSI == 1 )THEN  
          DO LP = 1,LLWET(K,ND)
            ...
            DO NAL = 1,NALGAE
              IF( ALGAES(NAL).ISILICA )THEN
                WQKK(L) = WQKK(L) + (ALGAES(NAL).WQFSID*WQBM(L,NAL) + ALGAES(NAL).WQFSIP*WQPR(L,NAL) - WQPA(L,NAL)) * ALGAES(NAL).WQASC * WQO(L,22+NAL)
              ENDIF
            ENDDO
            ! ***      PT_SRC_LOADS      VOLUME  
      ...
      IF( ISKINETICS(16) == 1 )THEN 
        ...
        DO LP = 1,LLWET(K,ND)
          ...
          DO NAL = 1,NALGAE
            ...
            WQA19A(NAL) = WQA19A(NAL)*WQO(L,22+NAL)                                      ! *** Net O2 production by group
          ...
          WQRea = DTWQO2*WQP16(L)*WQV(L,K,IDOX)    ! *** Reaeration - Offsetting Flux (WQP16 is negative)
          !WQDOC = WQAOCR*WQKHR(L)*WQO(L,IDOC)      ! *** DOC
          WQDOC = WQAOCR*WQKHR(L)*WQO(L,IDOC)/(WQKHORDOC*2+WQO(L,IDOC)+1.E-18)      !SWWCGHG: Michaelis-Menten equation of DOC
          !WQNH3 = WQAONT*WQNIT(L)*WQO(L,INHX)      ! *** Ammonia
          WQNH3 = 32./14.*1.5*WQNH4NO2(L) + 32./14.*0.5*WQNO2NO3(L)   ! ***O2 consumed by Nitrification SWWN2O
          WQCOD = WQO18(L)*WQO(L,ICOD)             ! *** COD
          !WQRR(L) = WQV(L,K,IDOX) + WQSUM  + WQCOD  + WQRea + DTWQO2*(WQA19 - WQDOC - WQNH3)
          WQRR(L) = WQV(L,K,IDOX) + WQSUM + WQCOD + WQRea - DTWQ*(WQDOC+WQNH3) + DTWQO2*WQA19 - (64./16.)*DTWQ*WQKCH4*O2WQ(L)/(WQKHORDO + O2WQ(L) + 1.E-18)*WQV(L,K,IFCB)/(WQKHCH4 + WQV(L,K,IFCB) + 1.E-18) !SWWCGHG: Add in CH4 oxidation
                                                                                                
          IF( IWQZPL > 0 ) WQRR(L) = WQRR(L) + DTWQ*SDOZ(L,K)    ! *** Coupling to zooplankton

          WQV(L,K,IDOX) = WQRR(L)*WQKK(L)
      ...
      IF( ISKINETICS(17) == 1 )THEN  
        IF( IWQSRP == 1 )THEN  
          ...
          DO LP = 1,LLWET(K,ND)
            L = LKWET(LP,K,ND)  
            WQV(L,K,ITAM) = WQRR(L)*WQKK(L)
            WQO(L,ITAM)   = WQVO(L,K,ITAM) + WQV(L,K,ITAM)
          ENDDO  
        ENDIF  
      ENDIF  

      ! ****  PARAM 18  CH4 - METHANE SWWCGHG
      IF( ISKINETICS(18) == 1 )THEN  
        DO LP=1,LLWET(K,ND)
          L=LKWET(LP,K,ND)
          WQRR(L) = 0
          IF( K == KSZ(L) )THEN            ! *** Note - CAN ADD A PER CELL ARRAY OF 1 TO KC WHERE K=KSZ IS 1.0 AND ALL OTHER LAYERS = 0.0   KSZ-指定网格最底部活动层的层序数
            WQRR(L) = WQRR(L) + SMJAQCH4(L)*HPKI(L,K)*0.5 ! *** Add in Benthic Flux
          ENDIF
          WQRR(L) = WQRR(L) - WQKCH4 * O2WQ(L)/(WQKHORDO + O2WQ(L) + 1.E-18) * WQV(L,K,IFCB)/(WQKHCH4 + WQV(L,K,IFCB) + 1.E-18)
          IF( K == KC )THEN  ! *** CH4通量 g/(m2·day)
            WQMTHFX(L) = WQMTHFX(L) + (-WQP18(L)*(WQV(L,K,IFCB)-WQMTHS(L)) * HPK(L,KC) + SMJGCH4(L)*0.5) * DTWQ
            WQRR(L) = WQRR(L) + WQP18(L)*(WQV(L,K,IFCB)-WQMTHS(L))
          ENDIF
          WQV(L,K,IFCB) = WQV(L,K,IFCB) + DTWQ*WQRR(L)
          WQV(L,K,IFCB) = MAX(WQV(L,K,IFCB), 0.0)
		ENDDO
	  ENDIF
      ...
      ! ****  PARAM 19 CO2 - DISSOLVED CARBON DIOXIDE
      ...
      IF( ISKINETICS(19) == 1)THEN  
        ...
        DO LP = 1,LLWET(K,ND)
          ...
          DTWQxH  = DTWQ*HPK(L,K)  
          DTWQxH2 = DTWQO2*HPK(L,K)
          Ka0=1/(1+10.**(WQPH(INT(TIMEDAY))-6.35)+10.**(2*WQPH(INT(TIMEDAY))-16.68)) !SWWCGHG: Coefficient to convert TCO2 to CO2
          IF( WQI0  <=  0.001 )THEN
            DO NAL = 1,NALGAE
              IF( ALGAES(NAL).ISMOBILE )THEN
                WQTTAA(NAL) = 0.0
          ...
          ELSE
            DO NAL = 1,NALGAE
              ...
              IF( ALGAES(NAL).ISMOBILE )THEN
                !WQTTAA(NAL) = (1.3 - 0.3*WQPN(L,NAL)) * WQPA(L,NAL) 
                WQTTAA(NAL) = WQPA(L,NAL)  !SWWCGHG: Modification of the CO2 consumption from photosynthesis
              ENDIF
            ENDDO
          ENDIF
          ...
          DO NAL = 1,NALGAE
            IF( ALGAES(NAL).ISMOBILE )THEN
              ...
              WQA22 = WQA22 + 3.67*WQA22A(NAL)*WQO(L,22+NAL)  !VB 3.67 CONVERTS g CARBON TO g CO2
            ENDIF
          ENDDO
          ...
          !WQCDRea = WQP19(L)*WQV(L,K,ICO2)                     ! *** Reaeration
          WQCDRea = WQP19(L)*Ka0*WQV(L,K,ICO2)                  !SWWCGHG: *ka0 to convert TCO2 to CO2
          !WQCDDOC = (WQKHR(L) + WQNIT(L))*WQO(L,IDOC)*3.67    ! *** DOC FROM HYDROLYSIS AND DENITRIFICATION 3.67 CONVERTS G CARBON TO G CO2    
          WQCDDOC = WQKHR(L)*WQO(L,IDOC)*3.67/(WQKHORDOC*2+WQO(L,IDOC)+1.E-18) + 0.5*44./14.*WQNO3NO2(L) + 0.5*44./14.*WQNO2N2OD(L) + 0.5*WQN2ON2(L)  !SWWN2O: CO2 source from HR+DEN
          !WQRR(L) = WQV(L,K,ICO2) + WQCDSUM + DTWQO2*(-WQA22 + WQCDDOC + WQCDRea)
          WQRR(L) = WQV(L,K,ICO2) + WQCDSUM + DTWQ*(WQCDDOC+WQCDRea) - DTWQO2*WQA22  + (44./16.)*DTWQ*WQKCH4*O2WQ(L)/(WQKHORDO + O2WQ(L) + 1.E-18)*WQV(L,K,IFCB)/(WQKHCH4 + WQV(L,K,IFCB) + 1.E-18)  !SWWCGHG: Add in CH4 oxidation
          IF( K == KSZ(L) )THEN !SWWCGHG: Add in Benthic Flux
            WQRR(L) = WQRR(L) + (44./12.*SMDGFCCO2(L)+44./32.*SMJCH4CO2(L) + 44./14.*SMJDEN(L))*HPKI(L,K)*DTWQ
          ENDIF
          
          !SWWCGHG: Calculate the flux of Denit, HR, CH4oxid and Algae processes
          TDENIT(L) = TDENIT(L) +  (0.5*44./14.*WQNO3NO2(L) + 0.5*44./14.*WQNO2N2OD(L) + 0.5*WQN2ON2(L)) * HPK(L,K)  * DTWQ
          THR(L) = THR(L) + WQKHR(L)*WQO(L,IDOC)*3.67/(WQKHORDOC*2+WQO(L,IDOC)+1.E-18) * HPK(L,K) * DTWQ
          TCH4OXID(L) = TCH4OXID(L) + (44./16.)*WQKCH4 * O2WQ(L)/(WQKHORDO + O2WQ(L) + 1.E-18) * WQV(L,K,IFCB)/(WQKHCH4 + WQV(L,K,IFCB) + 1.E-18) * HPK(L,K) * DTWQ
          IF(WQRR(L)<0.)THEN
            TALGAE(L) = TALGAE(L) + 0.5*WQA22 * HPK(L,K) * DTWQ + WQRR(L) * HPK(L,K)
          ELSE
            TALGAE(L) = TALGAE(L) + 0.5*WQA22 * HPK(L,K) * DTWQ
          ENDIF
          IF( K == KC )THEN  !SWWCGHG: CO2 flux g/(m2·day)
            WQCDFX(L) = WQCDFX(L) + (-WQP19(L)*(Ka0*WQV(L,KC,ICO2)-WQCDOS(L)) * HPK(L,KC) * DTWQ)
          ENDIF
          
          !WQV(L,K,ICO2) = MAX(WQRR(L)*WQKK(L),0.0)
          WQV(L,K,ICO2) = MAX(WQRR(L),0.0) !SWWCGHG: Delete the WQKK
        ENDDO
      ENDIF
      
      ! ****  PARAM 20  NO2- - Nirite nitrogen  g N/(m3·d)  SWWN2O
      IF( ISKINETICS(20) == 1 )THEN  
        DO LP=1,LLWET(K,ND)
          L=LKWET(LP,K,ND)
          WQRR(L) = 0
		  
          WQRR(L) = WQRR(L) + WQNH4NO2(L) !NH4-NO2
		  WQRR(L) = WQRR(L) - WQNO2N2ON(L)+WQNO2N2OD(L) !NO2-N2O 
		  WQRR(L) = WQRR(L) - WQNO2NO3(L) !NO2-NO3
		  WQRR(L) = WQRR(L) + WQNO3NO2(L) !NO3-NO2
		  
          WQV(L,K,INO2) = WQV(L,K,INO2) + DTWQ*WQRR(L)
          WQV(L,K,INO2) = MAX(WQV(L,K,INO2), 0.0)
		ENDDO
      ENDIF
      
      ! ****  PARAM 21  N2O - Nitrous oxide  g N2O/(m3·d)  SWWN2O
      IF( ISKINETICS(21) == 1 )THEN  
        DO LP=1,LLWET(K,ND)
          L=LKWET(LP,K,ND)
          WQRR(L) = 0
		  
          IF( K == KSZ(L) )THEN
            WQRR(L) = WQRR(L) + SMJAQN2O(L)*HPKI(L,K)*44./14. ! *** Add in Benthic Flux
          ENDIF
		  
		  WQRR(L) = WQRR(L) + 0.75*44./14.*WQNO2N2ON(L)+0.5*44./14.*WQNO2N2OD(L) !NO2-N2O 
		  WQRR(L) = WQRR(L) - WQN2ON2(L) !N2O-N2
		  TN2ON2(L) = TN2ON2(L) +  WQN2ON2(L) * HPK(L,K)  * DTWQ
		  TNO2N2ON(L) = TNO2N2ON(L) +  WQNO2N2ON(L) * HPK(L,K)  * DTWQ
		  TNO2N2OD(L) = TNO2N2OD(L) +  WQNO2N2OD(L) * HPK(L,K)  * DTWQ
          
          IF( K == KC )THEN  ! *** N2O通量 g/(m2·day)
            WQN2OFX(L) = WQN2OFX(L) + (-WQP21(L)*(WQV(L,K,IN2O)-WQN2OS(L))*HPK(L,KC))*DTWQ
            WQRR(L) = WQRR(L) + WQP21(L)*(WQV(L,K,IN2O)-WQN2OS(L))
          ENDIF
		  
          WQV(L,K,IN2O) = WQV(L,K,IN2O) + DTWQ*WQRR(L)
          WQV(L,K,IN2O) = MAX(WQV(L,K,IN2O), 0.0)
		ENDDO
      ENDIF
      
      ! ****  PARAM 22  N2 - Nitrogen gas  g N2/(m3·d)  SWWN2O
      IF( ISKINETICS(22) == 1 )THEN  
        DO LP=1,LLWET(K,ND)
          L=LKWET(LP,K,ND)
          WQRR(L) = 0
		  
          IF( K == KSZ(L) )THEN
            WQRR(L) = WQRR(L) + SMJAQN2(L)*HPKI(L,K) ! *** Add in Benthic Flux
          ENDIF
		  
          WQRR(L) = WQRR(L) + 28./44.*WQN2ON2(L) !N2O-N2
          
		  IF( K == KC )THEN  ! *** N2通量 g/(m2·day)
            WQN2FX(L) = WQN2FX(L) + (-WQP22(L)*(WQV(L,K,IN2)-WQN2S(L)) * HPK(L,KC)) * DTWQ
            WQRR(L) = WQRR(L) + WQP22(L)*(WQV(L,K,IN2)-WQN2S(L))
          ENDIF
          WQV(L,K,IN2) = WQV(L,K,IN2) + DTWQ*WQRR(L)
          WQV(L,K,IN2) = MAX(WQV(L,K,IN2), 0.0)
		ENDDO
	  ENDIF
    ...
    IF( IWQBEN == 1 )THEN  
      DO LP = LF,LL
        ...
        DO NAL = 1,NALGAE
          IF( ALGAES(NAL).ISMOBILE )THEN
            ! *** Phytoplankton
            WQDFB(L,NAL) = SCB(L)*ALGAES(NAL).WQWS(IMWQZ)*WQV(L,KSZ(L),22+NAL) 
          ELSE
            WQDFB(L,NAL) = ALGAES(NAL).WQWS(IMWQZ)*HPKI(L,K)*WQV(L,KSZ(L),22+NAL)
          ENDIF
        ENDDO       
        ...
        IF( IWQSI == 1 ) WQDFSI(L) = SCB(L)*ALGAES(2).WQWS(IMWQZ)*WQV(L,KSZ(L),ISUU)  
      ENDDO  
      IF( IWQSRP == 1 )THEN  
        DO LP = LF,LL
          L = LWET(LP)
  ...
  !SWWCGHG: Output of the GHG
  IF (countDay /= INT(TIMEDAY))THEN
    DO L=2,LA
      WRITE(11976,'(2I6,F17.5,F14.2,2F12.6,I10,6F14.6)') IL(L),JL(L),WQCDFX(L),WQKW19(L),Ka0*WQV(L,KC,ICO2),WQCDOS(L),INT(TIMEDAY)-1,WQPH(INT(TIMEDAY)),SMDGFCCO2(L),TCH4OXID(L),TDENIT(L),THR(L),TALGAE(L)
      WRITE(76119,'(2I6,F17.5,F14.2,2F12.6,I10,4F14.6)') IL(L),JL(L),WQMTHFX(L),WQKW18(L),WQV(L,KC,IFCB),WQMTHS(L),INT(TIMEDAY)-1,0.5*SMJGCH4(L),0.5*SMJAQCH4(L),SMCH4SAT(L),SMCH4REF(L)
      WRITE(1999 ,'(2I6,F17.5,F14.2,2F12.6,I10,4F14.6)') IL(L),JL(L),WQN2OFX(L),WQKW21(L),WQV(L,KC,IN2O),WQN2OS(L),INT(TIMEDAY)-1,SMJAQN2O(L),TNO2N2ON(L),TNO2N2OD(L),TN2ON2(L)
      WRITE(0706 ,'(2I6,F17.5,F14.2,2F12.6,I10,F14.6)') IL(L),JL(L),WQN2FX(L),WQKW22(L),WQV(L,KC,IN2),WQN2S(L),INT(TIMEDAY)-1,SMJAQN2(L)
    ENDDO
    !NH
    WRITE(16881,'(2I6,F17.6,F14.2,2F12.6,I10,6F14.6)') IL(1688),JL(1688),WQCDFX(1688),WQKW19(1688),Ka0*WQV(1688,KC,ICO2),WQCDOS(1688),INT(TIMEDAY)-1,WQPH(INT(TIMEDAY)),SMDGFCCO2(1688),SMJCH4CO2(1688),WQV(1688,KC,INO2),WQV(1688,KC,INO),WQV(1688,KC,IN2O)
	WRITE(16882,'(2I6,F17.6,F14.2,2F12.6,I10,4F14.6)') IL(1688),JL(1688),WQMTHFX(1688),WQKW18(1688),WQV(1688,KC,IFCB),WQMTHS(1688),INT(TIMEDAY)-1,0.5*SMJGCH4(1688),0.5*SMJAQCH4(1688),SMCH4SAT(1688),SMCH4REF(1688)
    WRITE(16883,'(2I6,F17.6,F14.2,2F12.6,I10,F14.6,I14,5F14.6)') IL(1688),JL(1688),WQN2OFX(1688),WQKW21(1688),WQV(1688,KC,IN2O),WQN2OS(1688),INT(TIMEDAY)-1,SMJAQN2O(1688),KSZ(1688),WQV(1688,25,IN2O),WQV(1688,20,IN2O),WQV(1688,15,IN2O),WQV(1688,10,IN2O),WQV(1688,KSZ(1688),IN2O)
    WRITE(16884,'(2I6,F17.6,F14.2,2F12.6,I10,F14.6)') IL(1688),JL(1688),WQN2FX(1688),WQKW22(1688),WQV(1688,KC,IN2),WQN2S(1688),INT(TIMEDAY)-1,SMJAQN2(1688)
    !HW
    WRITE(27641,'(2I6,F17.6,F14.2,2F12.6,I10,9F14.6)') IL(2764),JL(2764),WQCDFX(2764),WQKW19(2764),Ka0*WQV(2764,KC,ICO2),WQCDOS(2764),INT(TIMEDAY)-1,WQPH(INT(TIMEDAY)),SMDGFCCO2(2764),SMJCH4CO2(2764),TCH4OXID(2764),TDENIT(2764),THR(2764),TALGAE(2764),(WQCDFX(1688)+WQCDFX(2764)+WQCDFX(3312)+WQCDFX(2604)+WQCDFX(2854))/5.
	WRITE(27642,'(2I6,F17.6,F14.2,2F12.6,I10,4F14.6)') IL(2764),JL(2764),WQMTHFX(2764),WQKW18(2764),WQV(2764,KC,IFCB),WQMTHS(2764),INT(TIMEDAY)-1,0.5*SMJGCH4(2764),0.5*SMJAQCH4(2764),SMCH4SAT(2764),SMCH4REF(2764)
    WRITE(27643,'(2I6,F17.6,F14.2,2F12.6,I10,5F14.6)') IL(2764),JL(2764),WQN2OFX(2764),WQKW21(2764),WQV(2764,KC,IN2O),WQN2OS(2764),INT(TIMEDAY)-1,SMJAQN2O(2764),WQV(2764,KC,INO2),TNO2N2ON(2764),TNO2N2OD(2764),TN2ON2(2764)
    WRITE(27644,'(2I6,F17.6,F14.2,2F12.6,I10,F14.6)') IL(2764),JL(2764),WQN2FX(2764),WQKW22(2764),WQV(2764,KC,IN2),WQN2S(2764),INT(TIMEDAY)-1,SMJAQN2(2764)
    WRITE(27645,'(3I6,12F14.6)') IL(2764),JL(2764),INT(TIMEDAY)-1,SMPOC(2764,1),SMPOC(2764,2),SMPON(2764,1),SMPON(2764,2),SM1NH4(2764),SM2NH4(2764),SM1NO3(2764),SM2NO3(2764),WQBFNH4(2764),WQBFNO3(2764),WQBFO2(2764),XSMO20(2764)
    !DB
    WRITE(33121,'(2I6,F17.6,F14.2,2F12.6,I10,3F14.6)') IL(3312),JL(3312),WQCDFX(3312),WQKW19(3312),Ka0*WQV(3312,KC,ICO2),WQCDOS(3312),INT(TIMEDAY)-1,WQPH(INT(TIMEDAY)),SMDGFCCO2(3312),SMJCH4CO2(3312)
	WRITE(33122,'(2I6,F17.6,F14.2,2F12.6,I10,4F14.6)') IL(3312),JL(3312),WQMTHFX(3312),WQKW18(3312),WQV(3312,KC,IFCB),WQMTHS(3312),INT(TIMEDAY)-1,0.5*SMJGCH4(3312),0.5*SMJAQCH4(3312),SMCH4SAT(3312),SMCH4REF(3312)
    WRITE(33123,'(2I6,F17.6,F14.2,2F12.6,I10,F14.6)') IL(3312),JL(3312),WQN2OFX(3312),WQKW21(3312),WQV(3312,KC,IN2O),WQN2OS(3312),INT(TIMEDAY)-1,SMJAQN2O(3312)
    WRITE(33124,'(2I6,F17.6,F14.2,2F12.6,I10,F14.6)') IL(3312),JL(3312),WQN2FX(3312),WQKW22(3312),WQV(3312,KC,IN2),WQN2S(3312),INT(TIMEDAY)-1,SMJAQN2(3312)
    !HYD
    WRITE(26041,'(2I6,F17.6,F14.2,2F12.6,I10,3F14.6)') IL(2604),JL(2604),WQCDFX(2604),WQKW19(2604),Ka0*WQV(2604,KC,ICO2),WQCDOS(2604),INT(TIMEDAY)-1,WQPH(INT(TIMEDAY)),SMDGFCCO2(2604),SMJCH4CO2(2604)
	WRITE(26042,'(2I6,F17.6,F14.2,2F12.6,I10,4F14.6)') IL(2604),JL(2604),WQMTHFX(2604),WQKW18(2604),WQV(2604,KC,IFCB),WQMTHS(2604),INT(TIMEDAY)-1,0.5*SMJGCH4(2604),0.5*SMJAQCH4(2604),SMCH4SAT(2604),SMCH4REF(2604)
    WRITE(26043,'(2I6,F17.6,F14.2,2F12.6,I10,F14.6,I14,6F14.6)') IL(2604),JL(2604),WQN2OFX(2604),WQKW21(2604),WQV(2604,KC,IN2O),WQN2OS(2604),INT(TIMEDAY)-1,SMJAQN2O(2604),KSZ(2604),WQV(2604,25,IN2O),WQV(2604,20,IN2O),WQV(2604,15,IN2O),WQV(2604,10,IN2O),WQV(2604,5,IN2O),WQV(2604,KSZ(2604),IN2O)
    WRITE(26044,'(2I6,F17.6,F14.2,2F12.6,I10,F14.6)') IL(2604),JL(2604),WQN2FX(2604),WQKW22(2604),WQV(2604,KC,IN2),WQN2S(2604),INT(TIMEDAY)-1,SMJAQN2(2604)
    !BH
    WRITE(28541,'(2I6,F17.6,F14.2,2F12.6,I10,3F14.6)') IL(2854),JL(2854),WQCDFX(2854),WQKW19(2854),Ka0*WQV(2854,KC,ICO2),WQCDOS(2854),INT(TIMEDAY)-1,WQPH(INT(TIMEDAY)),SMDGFCCO2(2854),SMJCH4CO2(2854)
	WRITE(28542,'(2I6,F17.6,F14.2,2F12.6,I10,4F14.6)') IL(2854),JL(2854),WQMTHFX(2854),WQKW18(2854),WQV(2854,KC,IFCB),WQMTHS(2854),INT(TIMEDAY)-1,0.5*SMJGCH4(2854),0.5*SMJAQCH4(2854),SMCH4SAT(2854),SMCH4REF(2854)  
    WRITE(28543,'(2I6,F17.6,F14.2,2F12.6,I10,F14.6,I14,5F14.6)') IL(2854),JL(2854),WQN2OFX(2854),WQKW21(2854),WQV(2854,KC,IN2O),WQN2OS(2854),INT(TIMEDAY)-1,SMJAQN2O(2854),KSZ(2854),WQV(2854,25,IN2O),WQV(2854,20,IN2O),WQV(2854,15,IN2O),WQV(2854,10,IN2O),WQV(2854,KSZ(2854),IN2O)
    WRITE(28544,'(2I6,F17.6,F14.2,2F12.6,I10,F14.6)') IL(2854),JL(2854),WQN2FX(2854),WQKW22(2854),WQV(2854,KC,IN2),WQN2S(2854),INT(TIMEDAY)-1,SMJAQN2(2854)
    DO L=2,LA
      TDENIT(L) = 0.0
      TN2ON2(L) = 0.0
      TNO2N2ON(L) = 0.0
